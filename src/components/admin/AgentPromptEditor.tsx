...
import { Sparkles, Tag, Lock, Save } from "lucide-react";
...
export function AgentPromptEditor({
  ...
  return (
    <div className="flex flex-col w-full">
      {/* Info banner */}
      <div className="shrink-0 mb-4 flex items-start gap-3 rounded-xl border border-edge bg-surface-raised shadow-sm px-4 py-3">
        <div className="mt-0.5 shrink-0 flex h-8 w-8 items-center justify-center rounded-lg bg-signal/10" aria-hidden="true">
        ...
        {planName && (
          <span className="ml-auto shrink-0 inline-flex items-center gap-1 rounded-full border border-amber-400/60 bg-amber-400/10 px-2.5 py-0.5 text-[11px] font-medium text-amber-700 dark:text-amber-400 whitespace-nowrap">
      ...
      {(detectVariables(content).length > 0 || detectBlocks(content).length > 0) && (
        <div className="shrink-0 mb-4 rounded-xl border border-edge bg-surface-raised shadow-sm px-4 py-3 space-y-3">
          <div className="flex items-center gap-1.5">
            <Tag className="h-3 w-3 shrink-0 text-ink-4" aria-hidden="true" />
            <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-4">Contenido detectado en el prompt</p>
          </div>
      ...
      {/* Editor */}
      <div className="relative rounded-t-xl border border-b-0 border-edge overflow-hidden bg-zinc-950">
        <Textarea
          value={content}
          ...
          className="min-h-[420px] rounded-none border-0 bg-transparent font-mono text-sm text-zinc-100 placeholder:text-zinc-600 resize-none focus-visible:ring-0 focus-visible:ring-offset-0 px-4 py-4 pb-8"
        ...
        <div className="pointer-events-none absolute bottom-2 right-3">
          <span
            className={cn(
              ...
              overLimit ? "text-destructive" : isWarning ? "text-amber-400" : "text-zinc-500"
            )}
          >
